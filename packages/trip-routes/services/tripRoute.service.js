...
const createNewTripRoute = async (
  departureTime,
  arrivalTime,
  trip_id,
  coach_id,
...
const getTripRoutes = async () => {
  try {
    ...
    const tripList = await axios.get(
      `${process.env.TRIP_SERVICE_URL}/trip/?1000`
    );
    const coachList = await axios.get(
      `${process.env.COACH_SERVICE_URL}/coach/list`
    );

    const linkedTrips = tripRouteList.reduce((acc, item) => {
      const trip = tripList.data.results.find((t) => t.id === item.trip_id);
      const coach = coachList.data.coachList.find(
        (c) => c.id === item.coach_id
      );
      if (trip && coach) {
        acc.push({
          id: item.id,
...
const searchTripRoute = async (origin, destination, departureDate) => {
  try {
    const tripRouteList = await tripRoutes.find({
      departureTime: { $regex: departureDate, $options: "i" },
    });
    const result = [];
    for (const tripRoute of tripRouteList) {
      const trip = (
        await axios.get(
          `${process.env.TRIP_SERVICE_URL}/trip/${tripRoute.trip_id}`
        )
      ).data.trip;
      if (
        trip.origin.includes(origin) &&
        trip.destination.includes(destination)
      ) {
        const coach = (
          await axios.get(
            `${process.env.COACH_SERVICE_URL}/coach/id/${tripRoute.coach_id}`
          )
        ).data.coach;
...
const getTripRouteById = async (id) => {
  console.log(
    "🚀 ~ file: tripRoute.service.js ~ line 138 ~ getTripRouteById ~ id",
    id
  );
  try {
